...
SELECT
    ...
    LATERAL (
        SELECT
          ...
          ((
		      0.219058829 * CASE WHEN ''{Medtag i risikoberegninger}'' IN (''Skadebeløb'',''Skadebeløb og værditab'') THEN n.areal_oversvoem_nutid_m2 * p.{f_price_t_agr_price} ELSE 0.0 END + 
              0.089925625   * CASE WHEN ''{Medtag i risikoberegninger}'' IN (''Skadebeløb'',''Skadebeløb og værditab'') THEN f.areal_oversvoem_fremtid_m2 * p.{f_price_t_agr_price} ELSE 0.0 END
